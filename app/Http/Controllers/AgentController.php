...
class AgentController extends Controller
{
    ...
    public function show($encryptedId)
    {
        ...
        $leased_properties = $agent->properties()->where('is_leased_sold',
            'Y')->where('property_category_id', 2)->where('is_active','Y')->where('branch_id', env('BRANCH_ID'))->orderBy('created_at', 'desc')->paginate(6);
        $sold_properties = $agent->properties()->where('is_leased_sold',
            'Y')->where('property_category_id', 1)->where('is_active','Y')->where('branch_id', env('BRANCH_ID'))->orderBy('created_at', 'desc')->paginate(6);
        $rent_properties = $agent->properties()->where('property_category_id', 2)->where('is_leased_sold',
            'N')->where('is_active','Y')->where('branch_id', env('BRANCH_ID'))->orderBy('created_at', 'desc')->paginate(6);
