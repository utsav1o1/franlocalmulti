...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Cache::has('instagram_post')){
            ...
            $instagram = \InstagramScraper\Instagram::withCredentials(new \GuzzleHttp\Client(), 'multidynamic.auburn', 'mdauburn123!', new Psr16Adapter('Files'));
            try {

                $instagram->login(); // will use cached session if you want to force login $instagram->login(true)
                $instagram->saveSession();  //DO NOT forget this in order to save the session, otherwise have no sense
                $account = $instagram->getAccount('multidynamic.auburn');
